...
pub fn part2_implementation(input: &str) -> String {
    ...
    let clean_grid = get_piped_grid(&grid);

    get_island_count(&clean_grid, &grid).to_string()
}
    ...
    while changed {
        ...
        for (neighbour_cell, previous) in neighbours {
            ...
            if *distance_cell == Pipe::Ground  {
                    set::<Pipe>(&mut new_grid, neighbour_cell.0, neighbour_cell.1, grid
                        .get(neighbour_cell.0, neighbour_cell.1)
                        .unwrap()
                        .clone());
                    changed = true;
            }
...
fn get_island_count(grid: &Grid<Pipe>, grid_pipe: &Grid<Pipe>) -> usize {
    ...
    let internal_island: Vec<Vec<(usize, usize)>> = non_path_list
        .iter()
        .filter(|&c| c.iter().all(|&(row, column)| row != 0 && column != 0))
        .map(|c| {
            let mut vec = vec![];
            for elem in c {
                vec.push(*elem);
    ...
    internal_island.iter().map(|island| island.len()).sum()
}
